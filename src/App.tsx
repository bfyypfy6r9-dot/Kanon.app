...
export default function App() {
  ...
  const [viewMode, setViewMode] = useState<"sermon" | "admin">("sermon");
  ...
  const handleBuildSermon = async (format: "doc" | "pdf") => {
    ...
    try {
      ...
      const combinedResult = await res.json();
            ...
            {currentUser ? (
              <div className="bg-white/5 border border-white/10 p-4 mb-6" id="active_user_panel">
                ...
                {currentUser.role === "admin" && (
                  <button
                    type="button"
                    onClick={() => setViewMode(viewMode === "admin" ? "sermon" : "admin")}
                    className="w-full flex items-center justify-center gap-2 text-[10px] py-2.5 px-3 border border-[#D4AF37]/50 hover:bg-[#D4AF37]/10 text-[#D4AF37] font-sans font-bold uppercase tracking-widest transition-colors cursor-pointer mb-3"
                  >
                    <Shield className="w-3.5 h-3.5" />
                    {viewMode === "admin" ? "Painel de Sermão" : "Painel Administrador"}
                  </button>
                )}
              ...
              <div className="bg-white/5 p-5 border border-white/10" id="login_form_container">
                ...
                <form onSubmit={handleAuthSubmit} className="space-y-4" id="credentials_form">
                  <div>
                    ...
                    <input type="email" required value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} placeholder="pastor@exemplo.com" className="w-full bg-white/5 border border-white/20 p-2.5 text-xs text-white placeholder-white/20 font-sans focus:outline-none focus:border-white/50 transition-colors" />
                  </div>
                  <div>
                    <label className="block text-[9px] uppercase tracking-wider text-white/65 font-sans mb-1.5">Senha</label>
                    <input type="password" required value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} placeholder="••••••••" className="w-full bg-white/5 border border-white/20 p-2.5 text-xs text-white placeholder-white/20 font-sans focus:outline-none focus:border-white/50 transition-colors" />
                  </div>
                  {authError && (
                  ...
                  <button type="submit" disabled={authLoading} className="w-full bg-[#D4AF37] hover:bg-[#C19B2D] active:bg-[#AA8825] disabled:bg-neutral-800 text-black font-sans font-bold py-3 text-xs uppercase tracking-widest transition-all cursor-pointer flex items-center justify-center gap-2">
                    {authLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "Acessar Sistema"}
                  </button>
                </form>
              </div>
            )}
              ...
              <section className="bg-white border border-[#D1CEC5] p-6 shadow-sm" id="card_metadata">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="metadata_inputs_grid">
                  <div>
                    ...
                    <input type="text" value={passage} onChange={(e) => setPassage(e.target.value)} placeholder="Ex: Efésios 2:8-10" className="w-full bg-transparent border-b border-[#1A1A1A]/30 pb-2 text-sm text-[#1A1A1A] placeholder-[#1A1A1A]/40 focus:outline-[#D4AF37] transition-all font-serif" />
                  ...
                  <div>
                    ...
                    <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="Ex: Rev. Silva" className="w-full bg-transparent border-b border-[#1A1A1A]/30 pb-2 text-sm text-[#1A1A1A] placeholder-[#1A1A1A]/40 focus:outline-[#D4AF37] transition-all font-serif" />
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: A Suficiência da Graça" className="w-full bg-transparent border-b border-[#1A1A1A]/30 pb-2 font-serif text-lg font-bold text-[#1A1A1A] placeholder-[#1A1A1A]/40 focus:outline-[#D4AF37] transition-all" />
                    <p className="text-[10px] text-[#8B7E66] mt-1.5">O tema central ou ideia homilética norteadora.</p>
                  </div>
                </div>
              </section>

              <section className="bg-white border border-[#D1CEC5] p-6 shadow-sm" id="card_personalization">
                <h3 className="text-[10px] font-sans font-bold uppercase tracking-widest text-[#1A1A1A]/70 mb-6 flex items-center gap-2 pb-2 border-b border-[#D1CEC5]">
                  <FileText className="w-4 h-4 text-[#8B7E66]" />
                ...
                <div className="grid grid-cols-1 gap-6" id="personalization_inputs_grid">
                  <div>
                    ...
                    <input type="text" value={targetAudience} onChange={(e) => setTargetAudience(e.target.value)} placeholder="Ex: Jovens universitários, linguagem informal..." className="w-full bg-transparent border-b border-[#1A1A1A]/30 pb-2 text-sm text-[#1A1A1A] placeholder-[#1A1A1A]/40 focus:outline-[#D4AF37] transition-all font-serif" />
                  ...
                  <div>
                    ...
                    <textarea value={userDrafts} onChange={(e) => setUserDrafts(e.target.value)} placeholder="Cole aqui suas anotações, pensamentos ou direcionamentos para a IA..." className="w-full bg-transparent border border-[#1A1A1A]/30 p-3 text-sm text-[#1A1A1A] placeholder-[#1A1A1A]/40 focus:border-[#D4AF37] focus:outline-none transition-all font-serif min-h-[80px] resize-y" />
                    <p className="text-[10px] text-[#8B7E66] mt-1.5">Essencial para a IA incorporar seus pensamentos originais ao sermão gerado.</p>
                  </div>
                </div>
              </section>

              <div className="bg-white border border-[#D1CEC5] p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4" id="global_mode_control">
                <div>
                ...
                <div className="flex gap-2 text-[10px] font-sans shrink-0">
                  <button type="button" onClick={() => setGlobalMode("ai")} className={`px-3.5 py-2 cursor-pointer font-bold tracking-wider transition-all border ${globalMode === "ai" ? "bg-[#1A1A1A] text-white border-[#1A1A1A]" : "border-[#D1CEC5] text-[#1A1A1A] opacity-60 hover:opacity-100 bg-transparent"}`}>GERAR COM IA (COMPLETO)</button>
                  <button type="button" onClick={() => setGlobalMode("manual")} className={`px-3.5 py-2 cursor-pointer font-bold tracking-wider transition-all border ${globalMode === "manual" ? "bg-[#1A1A1A] text-white border-[#1A1A1A]" : "border-[#D1CEC5] text-[#1A1A1A] opacity-60 hover:opacity-100 bg-transparent"}`}>DIGITAR MANUALMENTE</button>
                </div>
              </div>

              <section className="space-y-4" id="card_structure">
                <div className="bg-white border border-[#D1CEC5] p-5 shadow-sm mb-4">
                ...
                <div className="border border-[#D1CEC5] p-5 bg-white shadow-sm flex flex-col transition-all hover:border-[#1A1A1A]" id="intro_config_wrap">
                  ...
                  {globalMode === "ai" ? (
                    ...
                    <textarea rows={3} value={introducaoText} onChange={(e) => setIntroducaoText(e.target.value)} placeholder="Digite a síntese ou introdução teológica manualmente..." className="w-full bg-[#F9F8F5] border border-[#D1CEC5]/40 p-3 text-xs focus:outline-[#D4AF37] focus:border-[#D4AF37] text-[#1A1A1A] font-serif leading-relaxed" />
                  )}
                </div>

                <div className="border border-[#1A1A1A] p-5 bg-white shadow-sm flex flex-col" id="dev_config_wrap">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    {globalMode === "ai" && (
                      <div className="flex items-center gap-2 border border-[#D1CEC5] px-2 py-1 bg-[#F9F8F5]">
                        ...
                        <select value={desenvolvimentoPoints} onChange={(e) => setDesenvolvimentoPoints(Number(e.target.value))} className="bg-transparent border-none text-[11px] font-bold font-sans cursor-pointer focus:outline-none text-[#1A1A1A]">
                  ...
                  {globalMode === "ai" ? (
                    ...
                    <textarea rows={4} value={desenvolvimentoText} onChange={(e) => setDesenvolvimentoText(e.target.value)} placeholder="Escreva os pontos estruturados do desenvolvimento aqui..." className="w-full bg-[#F9F8F5] border border-[#D1CEC5]/40 p-3 text-xs focus:outline-[#D4AF37] focus:border-[#D4AF37] text-[#1A1A1A] font-serif leading-relaxed" />
                  )}
                </div>
                ...
                <div className="border border-[#D1CEC5] p-5 bg-white shadow-sm flex flex-col transition-all hover:border-[#1A1A1A]" id="conclusion_config_wrap">
                  ...
                  {globalMode === "ai" ? (
                    ...
                    <textarea rows={3} value={conclusaoText} onChange={(e) => setConclusaoText(e.target.value)} placeholder="Digite a síntese final do sermão aqui..." className="w-full bg-[#F9F8F5] border border-[#D1CEC5]/40 p-3 text-xs focus:outline-[#D4AF37] focus:border-[#D4AF37] text-[#1A1A1A] font-serif leading-relaxed" />
                  )}
                </div>
                ...
                <div className="border border-[#D1CEC5] p-5 bg-white shadow-sm flex flex-col transition-all hover:border-[#1A1A1A]" id="apelo_config_wrap">
                  ...
                  {globalMode === "ai" ? (
                    ...
                    <textarea rows={3} value={apeloText} onChange={(e) => setApeloText(e.target.value)} placeholder="Digite a aplicação prática pastoral aqui..." className="w-full bg-[#F9F8F5] border border-[#D1CEC5]/40 p-3 text-xs focus:outline-[#D4AF37] focus:border-[#D4AF37] text-[#1A1A1A] font-serif leading-relaxed" />
                  )}
                </div>
              </section>
              ...
              <footer className="mt-8 pt-6 border-t border-[#D1CEC5] flex flex-col gap-4" id="main_trigger_card">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-end gap-4">
                  <div className="flex flex-wrap gap-3">
                    <button type="button" onClick={() => handleBuildSermon("doc")} disabled={generating} className="bg-[#1A1A1A] hover:bg-neutral-800 active:bg-black text-white px-5 py-4 font-sans font-bold text-xs uppercase tracking-wider transition-colors flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed border border-[#1A1A1A] shadow-sm select-none cursor-pointer">
                      {generating && generatingFormat === "doc" ? <><Loader2 className="w-4 h-4 animate-spin" /><span>Compilando DOC...</span></> : <><Download className="w-4 h-4 text-[#D4AF37]" /><span>Gerar Sermão em DOC</span></>}
                    </button>
                    <button type="button" onClick={() => handleBuildSermon("pdf")} disabled={generating} className="bg-[#D4AF37] hover:bg-[#C19B2D] active:bg-[#AA8825] text-black px-5 py-4 font-sans font-bold text-xs uppercase tracking-wider transition-colors flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed border border-[#D4AF37] shadow-sm select-none cursor-pointer">
                      {generating && generatingFormat === "pdf" ? <><Loader2 className="w-4 h-4 animate-spin" /><span>Compilando PDF...</span></> : <><FileCheck className="w-4 h-4 text-black" /><span>Gerar Sermão em PDF</span></>}
                    </button>
                  </div>
                </div>
              </footer>
            ...
            {(sermonResult || generating) && (
              <div className="lg:col-span-5 space-y-6" id="preview_col">
                <AnimatePresence mode="wait">
                  {generating ? (
                    <motion.div key="loading" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="bg-[#1A1A1A] text-[#F4F1EA] p-8 border-2 border-[#1A1A1A] flex flex-col justify-center items-center min-h-[450px]" id="loading_panel">
                      ...
                      {generationStage && (
                        <div className="text-[10px] font-sans text-[#D4AF37] uppercase tracking-widest font-bold mb-4 px-3 py-1.5 bg-white/5 border border-white/10">
                          {generationStage}
                        </div>
                      )}
                      <p className="text-white/60 text-xs max-w-sm leading-relaxed text-center font-serif mb-6">CONSTRUINDO</p>
                      <div className="w-full bg-[#F4F1EA]/10 border border-white/10 h-1.5 overflow-hidden max-w-[200px] mb-3">
                        <div className="bg-[#D4AF37] h-full w-2/3 animate-pulse"></div>
                      </div>
                      <span className="text-[9px] font-sans uppercase tracking-[0.15em] opacity-40">Processamento Hermenêutico RAG</span>
                    </motion.div>
                  ) : sermonResult ? (
                    <motion.div key="result" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="space-y-6" id="result_panel">
                      <div className="border-2 border-emerald-600 bg-emerald-50/50 p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <button onClick={handleDownloadDocx} className="w-full py-3 px-4 bg-[#1A1A1A] hover:bg-neutral-800 text-white font-sans font-bold text-xs uppercase tracking-widest transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-sm">
                            <Download className="w-4 h-4 text-[#D4AF37]" /> Baixar Word (.DOCX)
                          </button>
                          <button onClick={() => handleBuildSermon("pdf")} className="w-full py-3 px-4 bg-[#D4AF37] hover:bg-[#C19B2D] text-black font-sans font-bold text-xs uppercase tracking-widest transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-sm">
                            <FileCheck className="w-4 h-4 text-black" /> Baixar PDF
                          </button>
                        </div>
                      ...
                      <div className="border border-[#D1CEC5] bg-white shadow-sm overflow-hidden" id="tab_preview_container">
                        <div className="bg-[#F9F8F5] p-3 border-b border-[#D1CEC5]">
                          <div className="flex flex-wrap gap-1" id="preview_tab_header">
                            {["introducao", "desenvolvimento", "conclusao", "apelo", "referencias"].map(tab => (
                              <button key={tab} onClick={() => setActiveTab(tab)} className={`text-[9px] font-sans font-bold uppercase tracking-wider py-1.5 px-3 transition-colors cursor-pointer ${activeTab === tab ? "bg-[#1A1A1A] text-white" : "text-[#1A1A1A]/70 hover:bg-[#D1CEC5]/30"}`}>
                                {tab === "introducao" ? "1. Introdução" : tab === "desenvolvimento" ? "2. Desenv." : tab === "conclusao" ? "3. Conclusão" : tab === "apelo" ? "4. Apelo" : "Referências"}
                              </button>
                            ))}
                          </div>
                        </div>
